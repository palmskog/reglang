...
Lemma count_mem1 (T : eqType) (x : T) n (s : n.-tuple T) (i : 'I_n) :
  count_mem x s == 1 -> tnth s i = x -> index x s = i.
Proof.
  rewrite (tnth_nth x). case: i => i /= Hi. case: s => /= s /eqP Hs. subst.
  elim: s i Hi => //= a s IH i Hi. case: (ifP _) => /=.
  - rewrite add1n eqSS => /eqP-> /=. destruct i as [|i] => //.
    rewrite eqn0Ngt -has_count nth_cons => A B. case:notF.
    rewrite -(negbTE A) -B. apply/(has_nthP x). exists i => //=.
  - destruct i as [|i] => [/negbT /eqP //|/= A B C]. by rewrite (IH i).
Qed.
