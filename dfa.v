...
Section Emptyness.
  ...
  Definition dfa_inhab := exseq_dec delta_rc (fun x => x \in dfa_fin A).
  
  Lemma dfa_inhabP : reflect (exists x, x \in dfa_lang A) (dfa_inhab).
  Proof. 
    apply: (iffP idP); by [case/dec_eq => x; exists x|case => x ?; apply/dec_eq;exists x].
  Qed.
...
Lemma regular_quot (char : finType) (L1 L2 : lang char) :
...
Proof.
  ...
  - move => [x X1 X2]. exists x. change (x \in dfa_lang (dfa_op andb B C)).
    rewrite dfa_op_correct. apply/andP;split => //. exact/LB. 
  - move => [x]. rewrite -[X in is_true X -> _]/(x \in dfa_lang (dfa_op andb B C)).
    rewrite dfa_op_correct. case/andP => *. exists x => //. exact/LB.
Qed.
...
Section Pumping.
  ...
  Proof.
    ...
    - move => u [|c v] w // /eqP e _. exists 0 => /= H.
      ...
      rewrite -Hu -Hw catA cat_nseq_eq in H. case/(Lab_eq neq) : H.
